...
void* thread_convolute(void* arg){
    ...
    for (int row = args->startRow; row < args->endRow; row++){ //rowが(*args)の中のstartRowを指す
...
int main(int argc,char** argv){
    ...
    threadArgs[i].algorithm = &algorithms[type];
    threadArgs[i].startRow  = i * rowsPerThread;
    threadArgs[i].endRow    = (i == NUM_THREADS - 1) ? srcImage.height : (i + 1) * rowsPerThread;
    pthread_create(&threads[i], NULL, thread_convolute, &threadArgs[i]);
}
